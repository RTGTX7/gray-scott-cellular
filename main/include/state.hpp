...
namespace cadmium::celldevs::example::conway {
	...
	struct conwayState {
		...
		//! Default constructor function. By default, cells are dead.
		conwayState() : life(false) {
		}
	};

	//! It returns true if x and y are different.
	inline bool operator!=(const conwayState& x, const conwayState& y) {
	...
	[[maybe_unused]] void from_json(const nlohmann::json& j, conwayState& s) {
