		...
		[[nodiscard]] SIRState localComputation(SIRState state,
		  const std::unordered_map<std::vector<int>, NeighborData<SIRState, double>>& neighborhood) const override {
			...
			for (const auto& [neighborId, neighborData]: neighborhood) {
				...
				if(v == 1.0) {
					if(i == 0) { //First pass, store input 1
						if(s->state == 0) {
							newState = 0;
						} else if(s->state == 1) {
							newState = 1;
						...
						i++;
					} else { //Second pass, compute NOR
						if(newState == 2) {
							if(s->state == 1) {
								newState = 0;
							}
						} else {
							if(newState == 0 && s->state == 0) {
								newState = 1;
							} else if(newState == 0 && s->state == 1) {
								newState = 0;
							} else if(newState == 1 && s->state == 0) {
								newState = 0;
							} else if(newState == 1 && s->state == 1) {
								newState = 0;
							} else if(newState == 1 && s->state == 2) {
								newState = 0;
							} else if(newState == 0 && s->state == 2) {
								newState = 2;
							}
						}
					}
